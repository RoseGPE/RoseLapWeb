...
def make_plot(result, display_dir, overall_title="Chart Overall Title"):
  ...
  for trk in td:
    ...
    for i in range(len(trk['times'])):
      pts = pointsim.compute_points(trk['scoring'], trk['min_time'], trk['min_co2'], trk['times'][i][1], trk['co2s'][i])
      # print(trk['scoring'],trk['times'][i][2],pts)
      if not (points_total is None):
        points_total[i][1] = points_total[i][1] + pts
      compute_pts.append([trk['times'][i][0], pts])
...
if __name__ == "__main__":
  ...
  "track_data": 
    [{"ss": 0.0,
      "outputs": [],
      "name": "acceleration.dxf",
      "scoring": "acceleration",
      "min_time": 3.9,
      "min_co2": 3.5,
      'co2s': [0.06, 0.06, 0.06, 0.06, 0.06, 0.06],
      "times": [[0, 4.535479780152823], [1, 4.586620072621941], [2, 4.636908105321652], [3, 4.710605832043429], [4, 4.783040659074332], [5, 4.900067214751482]]},
    {"ss": 0.0, 
      "outputs": [],
      ...
      "min_co2": 3.5,
      'co2s': [0.06, 0.06, 0.06, 0.06, 0.06, 0.06],
      "times": [[0, 4.688394448521829], [1, 4.7179460751729625], [2, 4.746938123655725], [3, 4.789504222026006], [4, 4.831116629942896], [5, 4.898710900603592]]}],
  "vehicle": 
    {"co2_factor": 2.31,
    "drag_35mph": 44.0,
    "g": 32.2,
