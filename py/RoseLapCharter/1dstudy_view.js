...
app.title = 'title';
var selected_data = [...Array(data.length).keys()].map(i => true);
...
option = {
    ...
    yAxis: {
        type: 'value',
    min: Math.floor(Math.min.apply(Math,[...Array(data.length).keys()].map(i => selected_data[i] ? Math.min.apply(Math,data[i].map(a => a[1])) : Infinity )) ),
    max: Math.ceil( Math.max.apply(Math,[...Array(data.length).keys()].map(i => selected_data[i] ? Math.max.apply(Math,data[i].map(a => a[1])) : -Infinity )) ),
        axisLine: {
            onZero: false
        },
        name: 'Points',
